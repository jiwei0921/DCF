...
class Eval_thread():
    def __init__(self, loader, method='Ours', dataset='A', output_dir='./', cuda=torch.cuda.is_available()):
        super(Eval_thread, self).__init__()
        self.loader = loader
        self.method = method
    ...
    def run(self):
        ...
        msg = '[cost:{:.4f}s]{} dataset with {} method get {:.4f} mae, {:.4f} max-fmeasure, {:.4f} max-Emeasure, {:.4f} S-measure..'.format(time.time()-start_time, self.dataset, self.method, mae, max_f, max_e, s)
        msg_mae = mae
        return msg_mae, msg
    def Eval_mae(self):
