...
class calibration_dataset:
    def __init__(self, image_root, pos_root, neg_root, testsize):
        self.testsize = testsize
    ...
    def __getitem__(self,index):
        ...
        if name in os.listdir(self.pos_root):
            depth = self.binary_loader(self.pos_root+name)
            gt = torch.ones(1).to(dtype=torch.int64)
        elif name in os.listdir(self.neg_root):
            depth = self.binary_loader(self.neg_root+name)
            gt = torch.zeros(1).to(dtype=torch.int64)
        else:
            print("Fail for generate calibration depth in data.py")
            sys.exit()

        # data augment
        image, _, depth = cv_random_flip(image, depth, depth)
        image, _, depth = randomCrop(image, depth, depth)
        image, _, depth = randomRotation(image, depth, depth)
        image = colorEnhance(image)
...
def get_calibration_loader(image_root, pos_root, neg_root,batchsize, testsize,
